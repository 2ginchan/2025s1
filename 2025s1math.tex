    ...
    \begin{enumerate}
        ...
        \item $f(x)$は奇関数で，$-1$倍は連続性を保存するから，$x=0,1$について示す。
              $(i)$$x=0$での連続性
                          ...
                          まず，簡単な計算によって$x-x^3 \leq x \, (x \geq 0)$,$-(x-x^3) < -x \, (x < 0)$がわかる。よって任意の$x$で，$0 \leq |x-x^3|\leq |x|$である。ここで任意に正数$\varepsilon$を取れば，$0<|x-0|<\varepsilon$を満たす任意の$x$に対して，
                          \begin{align*}
                      ...
                      任意に正数$\varepsilon$を取る。$\delta = \min \{ 4\varepsilon/15 , 1/2 \}$ とすれば，$0<|x-1|< \delta$を満たす任意の$x$に対して，$|x-1| < 4\varepsilon /15$，$1/2 <x<3/2$である。このとき，
                      \begin{align*}
                      ...
                      以上から$x=-1,0,1$は$f$の連続点で，すなわち$S(f)$の元である。
    \end{enumerate}
\end{screen}
\begin{screen}
    \begin{enumerate}
        \item[(3)] $(i)$ $x \in \mathbb{R}\backslash \mathbb{Q}$のとき
            ...
            今$f$が$x$で連続であったと仮定する。すると$x$に収束する有理数列$\{x_n\}$に対して，$\{f(x_n)\}=\{x_n-{x_n}^3\}$も$f(x)=0$に収束せねばならない。
    \end{enumerate}
\end{screen}
\end{document}